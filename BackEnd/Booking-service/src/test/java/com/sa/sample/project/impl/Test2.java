...
//
        ...
        Booking booking = new Booking("ab", LocalDate.of(2021,10,10),LocalDate.of(2021,10,11),2,"no","aa","62a92e51b35c623b239e6488",10.0,"aa123");
        CreditCardDto creditCardDto = new CreditCardDto("ff","a","a","a",10.0,new Date(2021-10-10),10.0);
        when(bookingRepository.findById(bookingId)).thenReturn(Optional.of(booking));
        when(restTemplate.getForObject("http://localhost:8088/room/{roomId}" , Room.class,booking.getRoomId())).thenReturn(room);
        when(restTemplate.getForObject("http://localhost:9001/{creditId}", CreditCardDto.class, booking.getCreditCardId())).thenReturn(creditCardDto);
        ResponseEntityDTO responseEntityDTO = new ResponseEntityDTO(booking, room,creditCardDto);
